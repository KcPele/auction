...
export function Queue() {
  ...
  const approve = (a: Approval) => {
    const category = a.kind === "car" ? "CAR" : "GADGET";
    // API: POST /admin/listings/{category}/{id}/approve
    console.log(`Approve: /admin/listings/${category}/${a.id}/approve`);
    drop(a.id);
    setReviewing(null);
  };
  ...
  const reject = (a: Approval) => {
    ...
    console.log(`Reject: /admin/listings/${category}/${a.id}/reject`);
    drop(a.id);
    setReviewing(null);
  };
        ...
        <CardBody flush>
          {items.length === 0 ? (
            ...
            items.map((q) => (
              <div
                key={q.id}
                className={`flex flex-col gap-3 overflow-hidden border-b border-line px-3.5 py-3 transition-all duration-300 last:border-b-0 sm:flex-row sm:items-center sm:px-[18px] ${
              ...
              >
                ...
                <div className="flex flex-shrink-0 flex-wrap gap-1.5 sm:flex-nowrap">
                  ...
                  <button
                    type="button"
                    onClick={() => approve(q)}
                    className="flex-1 rounded-[5px] border border-green/30 bg-transparent px-2.5 py-1 text-[11px] font-semibold text-green hover:bg-green/10 sm:flex-initial"
                  >
                    Approve
                  </button>
                  <button
                    type="button"
                    onClick={() => reject(q)}
                    className="flex-1 rounded-[5px] border border-red/30 bg-transparent px-2.5 py-1 text-[11px] font-semibold text-red hover:bg-red/10 sm:flex-initial"
                  >
                    Deny
                  </button>
                </div>
